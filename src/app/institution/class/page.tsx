      ...
      <div className="w-full flex flex-col items-center mt-8">
        ...
        <div className="container mx-auto border bg-[#FFFFFF] w-[85%] h-[85%] p-8 pr-15 pt-20 pb-20 space-y-2 rounded-3xl dark:bg-black dark:border-black">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {loading ? (
              <p className="text-center text-gray-700 dark:text-white">Carregando turmas...</p>
            ) : error ? (
              <p className="text-center text-red-500">{error}</p>
            ) : displayedClasses.length === 0 ? (
              <p className="text-center text-gray-700 dark:text-white">Nenhuma turma encontrada.</p>
            ) : (
              displayedClasses.map((turma) => (
                <div key={turma.id} className="bg-blue-50 dark:bg-[#141414] p-4 rounded-lg shadow">
                  <h3 className="font-bold text-lg dark:text-white">
                    {turma.nomeTurma}{" "}
                    <span className="text-gray-500 text-sm dark:text-[#8A8A8A]">Nº{turma.id}</span>
                  </h3>
                  <p className="text-gray-700 dark:text-white">{turma.quantidadeAlunos} alunos ativos</p>
            ...
            <div className="flex justify-center mt-6">
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  ...
                  className={`px-4 py-2 mx-1 rounded-md transition ${currentPage === i + 1 ? "bg-blue-500 text-white dark:text-black" : "bg-[#F0F7FF] text-blue-500 hover:bg-gray-300 dark:bg-[#141414]"}`}
                >
                  {i + 1}
                </button>
              ))}
